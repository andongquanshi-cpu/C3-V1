...
4. **产品**服从植入强度：**none/medium** 只在痛点解决路径里轻点或可不写；**high** 口播约 40% 场景铺垫 + 60% 产品/功能组合说明。禁止通篇功能清单或开篇推销（high 除外，后段可展开组合说明）。
...
8. 合规：不推荐具体股票/基金/代码，不承诺收益；**风险提示口语融入最后一镜口播**（见上方「风险提示 · 口播化」），禁止每条都用同一句标准标语贴片。
9. tags 必填 8-10 个（不带 #），按赛道/主题/读者/Offer或功能/品牌分层。非 none 植入必须含 2-3 个品牌直连词，且所有话题必须能在口播中找到语义依据。
...

{
  ...
  "riskReminder": "口语化风险提示（须改写，勿照搬「市场有风险，投资需谨慎」原句；与最后一镜口播一致）",
  "qualityScore": {
    "overallScore": 0,
    "scores": {
      "speechNaturalness": 0,
