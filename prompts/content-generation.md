...
2. 标题生成 3 个候选，封面文案生成 3 个候选；标题优先吸引同类读者共鸣，不要像广告 slogan。可自然用平台口语（谁懂啊、听劝、救命级、求教程等），禁止堆砌烂梗。**热点解读时**标题须体现具体事件/政策/现象，禁止「我的第一反应是…」「先别急着冲」等空泛 Reaction 式标题。
3. 正文必须口语化、分段清晰、有小红书阅读感（第一人称或朋友聊天感）；若 generationMode 为 video-script，content 字段输出口播视频脚本（分镜+口播句），总时长控制在指定秒数内。结尾 **interactionGuide 必填**：从提问评论 / 投票二选一 / 接龙 / 悬念预告（下期）/ 收藏照做 中选一种写一句软互动，禁止空、禁止硬广。若主题适合系列感，可在标题或末句轻点「第N期/打工人理财周记」等（勿每篇硬凑）。
4. 产品/功能提及须服从上方「植入强度」与产品层级（平台→Offer→功能，**禁止写反**）：理财通示例为「理财通 → 固收+ → 严选专区」，勿写成「严选专区归拢固收+」.**none** 可不写产品；**medium** 故事为主，若提功能/Offer 则全文至少点一次平台名（后续功能不必重复平台）；**high** 前约 40% 铺垫 + 后约 60% 展开平台→主推产品→全部子功能（平台点清即可，勿每个功能复读平台名）。
...
8. 风险意识若需要，用一句口语嵌进正文末段即可；**禁止**文末单独贴「市场有风险，投资需谨慎」或 ⚠️ 贴片。riskReminder 可留空；若填写须按指引改写，禁止批量同句。
...
12. 如果批量生成，每条内容必须绑定不同 angleId，且不得复用相同开头句、正文结构、互动引导或 riskReminder 原句。
13. emoji 须按人设/场景区分密度与气质（见运行时「Emoji · 按人设/场景区分」）：日记/生活类偏多，观察/教学偏克制但仍要达标；以句中/句尾为主，最多 1-2 段段首起势；禁止每段段首 emoji，禁止 💼📝✅💡 当分段小标题。禁止「首先/其次/第一/第二」等可见结构标记；产品提及嵌在叙事中段（**high 档位**：后约 60% 篇幅展开产品/功能组合），禁止文末单独 👉 硬推导流。
14. imagePromptSuggestions 必填 1-3 条，每条 prompt 必须是可直接用于文生图的完整画面描述：竖版 3:4 信息流封面、主体场景、光线色调、构图；若有 selectedCoverText 须在 prompt 中写明「画面内封面大字仅一句：xxx」（不要写 coverText: 字段格式）。**禁止**在 prompt 里写「小红书」或引导生成平台 Logo/角标。禁止只写抽象概念或产品功能名。
...
输出 JSON 结构必须严格包含：
{
  ...
  "interactionGuide": "必填：结尾互动钩子一句（提问/投票/接龙/悬念预告/收藏引导）",
  "riskReminder": "一句改写后的口语风险提示小字（禁止照抄固定标语）",
