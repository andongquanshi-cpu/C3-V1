...
本批结构化创意坐标（每个角度必须分别服从对应编号；若与 Brief、场景、人设、合规冲突，以后者为准）：
{{diversityAxisPlan}}
...
5. 每个角度必须落实对应编号的结构化创意坐标，并填写 differentiationAxis，说明它主要靠什么和其他角度区分；若坐标与 Brief、创作场景、人设、事实或合规约束冲突，必须保留业务约束并只替换冲突坐标值。同一批次内 differentiationAxis 不得重复超过 1 次。
6. 角度之间至少在人群、情绪、叙事结构、核心观点中有 2 项不同；热点模式下禁止整批角度写成同一篇新闻复述，也禁止整批角度全部脱离热点。
...
14. 标题方向 titleDirections 应偏生活化/ curiosity / 共鸣，避免「XX 功能太强了」类硬广标题；可带平台口语气质（谁懂啊/听劝等），禁止堆砌烂梗；**禁止**「N步/N个点/分成X份」框架式标题；**热点解读时禁止**「我的第一反应是…」「先别急着冲」「跟我有什么关系」等空泛 Reaction 标题作为主方向——标题须含具体事件/政策/现象。titleDirections 只供后续成稿参考，不承担卡片标签展示。
15. contentStructure 须含结尾「互动引导」位（提问/投票/接龙/悬念预告择一）；批次中可有 1–2 个角度带轻系列感（如「打工人理财周记第N期」）或热点敏捷框（如「今天异动，我只做了三件事」），勿全批硬凑系列。
16. 每个角度必须给出风险等级：low / medium / high。
17. 只输出合法 JSON，不要输出 Markdown、解释文字或代码块。
