...
任务信息：
- 内容类型：{{contentType}}
- 用户主题（角度展开的叙事主线，优先服从此项）：{{topic}}
- 主热点参考（仅 1 个角度应深度使用，勿让所有角度都围绕此标题）：{{primaryHotspotReference}}
- 目标用户：{{targetUser}}
- 推广目标：{{campaignGoal}}
...
参考热点/素材（事实依据；按下方「热点权重分配」差异化使用，勿让每条角度都围绕同一标题）：
{{topicMaterials}}
...
2. 每个角度的 coreIdea 必须首先回答「这篇笔记读者为什么要看」——情绪、场景、观点或信息增量；不能把角度写成产品卖点标题或新闻标题复述。
3. 热点素材必须按「热点权重分配」差异化使用：禁止 {{generateCount}} 个角度都写成同一新闻解读；至多 1 个角度深度绑定主热点，其余角度以用户主题、人设、生活场景为主。productBridge 仅在有素材/主题关联时填写，且不得喧宾夺主。
...
6. 角度之间至少在人群、情绪、叙事结构、核心观点中有 2 项不同；不要 4 个角度都是「功能 A vs 功能 B」，也不要 4 个角度都是「解读同一政策/同一新闻」。
