...
export function buildBusinessLineRuntimeLock(
  ...
  if (isLicaitongLine(businessLine)) {
    ...
    return [
      ...
      "- 层级硬规则：腾讯理财通（平台）→ 固收+（主推 Offer）→ 严选专区等（固收+下的子功能）。禁止写成「严选专区把固收+产品归到一起」。",
      "- 若正文提到功能/Offer：至少点一次「腾讯理财通/理财通」锚定；后续功能可顺着写，禁止每个功能重复平台名，也禁止只写功能不提平台。",
      bodyRule,
  ...
  return [
    ...
    "- 若正文提到功能/Offer：至少点一次「腾讯微证券/微证券」锚定；后续功能可顺着写，禁止每个功能重复平台名，也禁止只写功能不提平台。",
    weisecBodyRule,
    narrativeLock,
    embedGuide,
  ].join("\n");
